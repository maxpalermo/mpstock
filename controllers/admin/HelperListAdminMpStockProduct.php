...
class HelperListAdminMpStockProduct extends HelperList
{
    ...
    private function getList()
    {
        ...
        if (!Tools::isSubmit('submitReset' . $this->table)) {
            $filter_id_mpstock_movement = (int) Tools::getValue($this->table . 'Filter_id_mpstock_movement', 0);
            ...
            $filter_stock_quantity_before = Tools::getValue($this->table . 'Filter_stock_quantity_before', '');
            $filter_stock_movement = Tools::getValue($this->table . 'Filter_stock_movement', '');
        ...
        if (Tools::isSubmit($this->table . 'Orderby')) {
            $page = 1;
            $pagination = 20;
            $filter_id_mpstock_movement = (int) $this->cookie->__get($this->table . 'Filter_id_mpstock_movement');
            ...
            $filter_stock_quantity_before = $this->cookie->__get($this->table . 'Filter_stock_quantity_before');
            $filter_stock_movement = $this->cookie->__get($this->table . 'Filter_stock_movement');
        ...
        $this->cookie->__set($this->table . 'Filter_id_mpstock_movement', $filter_id_mpstock_movement);
        ...
        $this->cookie->__set($this->table . 'Filter_stock_quantity_before', $filter_stock_quantity_before);
        $this->cookie->__set($this->table . 'Filter_stock_movement', $filter_stock_movement);
        ...
        $sql1->select('distinct p.id_mpstock_movement as `id`')
            ->select('p.id_document')
            ...
            ->select('p.ean13')
            ->select('p.upc')
            ->select('p.stock_quantity_before as `snap`')
            ->select('p.stock_movement as `qty`')
            ...
            ->leftJoin('mpstock_document', 'doc', 'doc.id_mpstock_document=p.id_document')
        ...
        $sql3->select('distinct ord.id_order_detail')
            ...
            ->where('ort.state=99999');

        if ($filter_id_mpstock_movement) {
            $sql_where[] = ('id=' . (int) $filter_id_mpstock_movement);
        }
        ...
        if ($result) {
            ...
            foreach ($result as &$row) {
                $document = $this->getDocument((int) $row['id_document'], $row['type_row'], $row['date_add']);
                $i++;
