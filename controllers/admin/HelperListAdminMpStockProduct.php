...
class HelperListAdminMpStockProduct extends HelperList
{
    ...
    private function getList()
    {
        ...
        if (!Tools::isSubmit('submitReset'.$this->table)) {
            ...
            $filter_physical_quantity = Tools::getValue($this->table.'Filter_physical_quantity', '');
            $filter_usable_quantity = Tools::getValue($this->table.'Filter_usable_quantity', '');
        ...
        if (Tools::isSubmit($this->table.'Orderby')) {
            ...
            $filter_physical_quantity = $this->cookie->__get($this->table.'Filter_physical_quantity');
            $filter_usable_quantity = $this->cookie->__get($this->table.'Filter_usable_quantity');
        ...
        $this->cookie->__set($this->table.'Filter_physical_quantity', $filter_physical_quantity);
        $this->cookie->__set($this->table.'Filter_usable_quantity', $filter_usable_quantity);
        ...
        $sql1->select('distinct p.id_mpstock_product as `id`')
            ...
            ->select('p.ean13')
            ->select('p.upc')
            ->select('p.physical_quantity as `snap`')
            ->select('p.usable_quantity as `qty`')
        ...
        PrestaShopLoggerCore::addLog("GET MOVEMENTS: ".$query);
        $result = $db->executeS($query);
        if ($result) {
            $i=0;
    ...
    public function getDocument($id_document, $type, $date_add)
    {
        ...
        if ($result) {
            ...
            return array(
                ...
                'date_document' => Tools::displayDate($date_add),
            );
        }
    }
